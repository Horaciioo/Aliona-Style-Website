...
import { SECTION_SPACING, TRANSITION } from '@/declarations/ui/tokens'
      ...
      <Container width="wide">
        ...
        <ol className={cn(CRAFT_STYLES.list, 'mt-24')}>
          {items.map((item, index) => (
            <li key={item.id}>
              <a href={`#${SECTION_ANCHORS.pricing}`} className={CRAFT_STYLES.item}>
                <span className={CRAFT_STYLES.index} aria-hidden="true">
                  {String(index + 1).padStart(INDEX_PAD, '0')}
                </span>
                <div className={CRAFT_STYLES.body}>
                  <Text
                    appearance="blockTitle"
                    as="h3"
                    className={cn(TRANSITION, 'group-hover:text-primary')}>
